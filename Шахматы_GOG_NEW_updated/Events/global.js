...
function moveElement(piece, id, castle) {
  ...
  if (!castle) {
    changeTurn();
    document.dispatchEvent(new CustomEvent('chessTurnChanged', { detail: { inTurn } }));
    document.dispatchEvent(new CustomEvent('chessPieceMoved', { detail: { piece, from, to: id } }));
  }
}
  ...
  ROOT_DIV.addEventListener("click", function (event) {
    if (event.target.localName === "img") {
      ...
      if (
        ...
        const pieceColor = moveState.piece_name.includes("WHITE") ? "white" : "black";
        if (pieceColor !== inTurn) {
          clearPreviousSelfHighlight(selfHighlightState);
          clearHighlightLocal();
          moveState = null;
          return;
...
export { GlobalEvent, movePieceFromXToY, moveElement, clearSelection };
